...
class DynamicVoice:
    ...
    __author__ = "Kowlin, changed a little by Young"
    ...
    async def _check_count(self, server):
        ...
        print('--- START CHECK COUNT FUNCTION ---')
        empty = []
        used = []
    ...
    async def check_voice(self, memb_before, memb_after):
        ...
        print('--- START JOIN FUNCTION ---')
        if memb_after.voice.voice_channel is not None:
            print('User joined: {}'.format(memb_after.voice.voice_channel))
            channel = memb_after.voice.voice_channel
            if channel.id not in cache:
                if channel.id in channels:
                    print('Passed check: Renaming Channel and adding in cache')
        ...
        print('--- START LEAVE FUNCTION ---')
        if len(memb_before.voice.voice_channel.voice_members) == 0:
            channel = memb_before.voice.voice_channel
            print('User left: {}'.format(memb_before.voice.voice_channel))
            if channel.id in cache:
                print('Channel in cache')
                if len(channel.voice_members) == 0:
                    print('Channel is in cache. And its empty.')
                    await self.bot.delete_channel(channel)
